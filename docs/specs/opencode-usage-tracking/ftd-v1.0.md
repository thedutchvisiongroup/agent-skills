...
| 1.2 | 2026-08-27 | Thim + AI agents | Deterministic hash project directories replace ULID + registry |
| 1.3 | 2026-08-29 | Thim + AI agents | Non-git projects no longer share one output directory: a filesystem-root worktree counts as absent in the project-identity preference |
...
### 11.4 Implementation notes (v1.0; j–m added in v1.1; (j) rewritten in v1.2; root-worktree rule added in v1.3)

Extensions and deviations of the implemented V1 versus the schemas above (a–i verified by the 49-test Bun suite; j–m added in v1.1, verified by the 69-test suite — (j) rewritten in v1.2 for deterministic hash naming, verified by the 77-test suite — (j) amended in v1.3 for the filesystem-root worktree rule, verified by the 85-test suite — see `.agents/runs/2026-08-26-opencode-usage-tracking/reports/` and `.agents/runs/2026-08-29-usage-tracking-projectdir-fix/reports/`):
...
- **(j) Deterministic hash project directories (v1.1; rewritten v1.2; root-worktree rule v1.3)** — project storage layout is `<outputRoot>/<hash>/` where hash = first 24 lowercase hex chars of sha256(hostname + "\n" + remoteKey); remoteKey = trimmed `git remote get-url origin` output with one trailing `.git` removed when the repo has an origin remote, else the resolved absolute worktree path (fallback directory path); a worktree that resolves to the filesystem root (`"/"`) counts as ABSENT in the identity preference, so the identity falls back to the resolved directory path (v1.3 bug fix: OpenCode passes `worktree = "/"` when started outside a git repository, so without this rule every non-git project on a machine hashed to one shared directory — observed live as `23be191393b51993adc296ed` holding 13 sessions from 3 project directories); a genuine directory of `"/"` remains a valid identity; hostname = `os.hostname()`, `"unknown"` when empty. The name is a pure deterministic function — no registry, no persisted state, stable across restarts, unique per (device, project), and merge-safe across devices (each device hashes its own hostname so the same repo never shares a directory). Documented caveats: different remote forms of the same origin (ssh vs https, trailing slash, case) hash differently (fragmentation on remote-protocol switch); two devices with EMPTY hostnames and identical paths would share a directory (stats merge via event-ID dedup — no corruption); moving a repo without an origin remote changes its directory; the directory name allows offline confirmation (not recovery) of guessed (hostname, remote) pairs — accepted by design. Old ULID/hash/commit-named directories and any `projects.json` are disposable leftovers.
